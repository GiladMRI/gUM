function [ history ] = iter_admmg(x_ref, iter_ops, llr_ops, lsqr_ops, Aop, b, callback_fun )
...
for ii=1:max_iter
    ...
    [a, ~, ~, lsqr_nitr] = symmlq(AHA_lsqr, b(:) + rho * (z(:) - u(:)), lsqr_ops.tol, lsqr_ops.max_iter, [], [], x(:)); 
%     AHA_lsqr2p=@(a,b) AHA_lsqr(a);
%     [a, ~, ~, lsqr_nitr] = lsqr(AHA_lsqr2p, b(:) + rho * (z(:) - u(:)), lsqr_ops.tol, lsqr_ops.max_iter, [], [], x(:)); 
    x = reshape(a, size(b));
    ...
    fprintf('%3d\t%10d\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.2f\t%s\n', ii, ...
        sum(history.lsqr_nitr), history.r_norm(ii), history.eps_pri(ii), ...
        history.s_norm(ii), history.eps_dual(ii), history.objval(ii),datestr(now));
