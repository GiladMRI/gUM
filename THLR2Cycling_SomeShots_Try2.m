...
X0aNoBx1=mean(perm94(perm73(X0aNoB)).*conj(TSCPerShotCorr),9);
X0aNoBx=repmat(X0aNoBx1,[1 1 1 1 1 1 1 1 nShotsTogether]);
...
for jj=1:nIters
    ...
    UpdatedX0WPerShotWMainB=X0Nr+alpha*rcur; % with perShotB, no mainB
...
%     fgmontage(B0PerShotaSm,[-4 4])
    ...
    if(~UseOnly1CS)
        Fitted2=FitToModel_2CSf(squeeze(UpdatedX0),H_AllTS4_3);
