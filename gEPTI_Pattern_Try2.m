...
% M=(1:PatternLen)+HalfN+((21:35).');
M=(1:PatternLen)+HalfN-PatternLen/2+((-7:7).');
WritePatternToFile("Pattern1.txt",M,N);
%%
% M=min((0:2:119)+((0:5).'),120);
M=min(N-1,(1:PatternLen)*2-1+HalfN-PatternLen+((-7:7).'));
WritePatternToFile("Pattern2.txt",M,N);
%%
nCovers=4;
MaxJump=10;
...
%%
WritePatternToFile("Pattern3.txt",COrdX,N);
