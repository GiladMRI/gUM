...
[G8ms{1}, S8ms{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,2.48,true,InnerShotTime_ms,40.5,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle);
...
[G12ms{1}, S12ms{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,4.02,true,InnerShotTime_ms,42.5,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle);
...
[G16ms{1}, S16ms{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,5.7,true,InnerShotTime_ms,42.5,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle);
...
[G24ms{1}, S24ms{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,9.1,true,InnerShotTime_ms,42,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle);
...
Order16ms=[0 1 2];
Order24ms=[0 1];
Orders={Order8ms Order12ms Order16ms Order24ms};
clear Grads
for r=1:numel(Grads11mm)
    Grads{r}=Row(Grads11mm{r}.*exp(1i*Orders{r}*InterShotPhiRads(r)));
...
disp('Saved');
%%
save('Grads11mmb.mat','Grads11mm','Grads');
%% That's it
