...
R=(linspace(0,1,nTrajBase).^VD)*(N/2);
...
AcqTimePoints_us=(0:AcqDwellTime_us:numel(g)*GradDwellTime_us);

CTraj=interp1((0:(size(kK,1)-1))*GradDwellTime_us,kK,AcqTimePoints_us);
nTrajTrg=numel(CTraj);
Traj=[real(CTraj); imag(CTraj)];
...
close all;
MxVecXXT{TrajIdx}={MxVecXX MxVecYY MxVecXY MxVecYX};
ImT{TrajIdx}=Im;
end
save('PSFs_analysisb.mat','ImT');
%%
figure;
subplot(3,4,1);
plot(Traj(1,:),Traj(2,:));
...
ShowCurrentRegriddedPSF;
%% Spiral nShots 
CurTtl='Spiral linear order ';
nInnerShots=12;
nLoops=1;
VD=0.6;
% VD=1;
CurTtl=[CurTtl num2str(nInnerShots) ' shots, VD=' num2str(VD) ', ' num2str(nLoops) ' loops'];
nPreShot=nTrajBase/nInnerShots;
R=(linspace(0,1,nPreShot).^VD)*(N/2);
Phi=linspace(0,2*pi*nLoops,nPreShot);
CTraj=(R.*exp(1i*Phi)).';

ShotPerm=[1:(nInnerShots/2); (1:(nInnerShots/2))+(nInnerShots/2)];
ShotPerm=ShotPerm(:).';
...
gprint(1,['FigPSF_' CurTtl]);
disp(['Made ' CurTtl]);
close all
%% Spiral nShots 
CurTtl='Spiral random order ';
nInnerShots=12;
nLoops=1.3;
VD=0.6;
% VD=1;
