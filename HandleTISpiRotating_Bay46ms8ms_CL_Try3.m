...
for rs=1 % :nRepsSets
    ...
    tmp=bart(['picsS -w 1 -S -m ' RhoStr ' -R K:64:3:' num2str(THLR_lambda) ':2:1:0:6 ' ScriptFN_AllTS],Sz16_THLR,NoNavB.*DataCCP(:,TrajPartMed,CurReps,1:nccToUse),...
...
UpdatedB0Map_RSx=SmoothByW(UpdatedB0Map_RS,grmss(s_vals_RS,3));
...
TSCMed(:,:,:)=exp(-TimePoints_Med_ms3./UpdatedT2SMap_ms_RS(:,:,1)).*exp(-1i*2*pi*UpdatedB0Map_RSx.*TimePoints_Med_ms3/1e3);
TSCxPMed(:,:,1,1,1,1,:)=permute(TSCMed(:,:,:,1),[1 2 7 6 5 4 3]);
...
%%
% dB0dy=symD(UpdatedB0Map_RSx,2);
% %
% PEPhaseDiffS=2*pi.*TimePoints_Med_ms3.*perm43(dB0dy)/1000;
% %
% kLoc=squeeze(STraj3MMed(2,floor(TimePointsR_Med*nTraj),RepsSets{2}));
% PhaseDiffDueToImaging= 2*pi*kLoc/Sz(2);
...
% disp('ok');
% % ScriptFN_CompgBo_dB0='/autofs/space/daisy_002/users/Gilad/gUM/nuftCompgB0_dB0_N.txt';
% %%
% CurRep=RepsSets{2};
% ShotNcc=17;
% ShotKerns=perm83(KernsPMMedx);
% ShotTSC=TSCxPMedOnlyB0.*PEPhaseDiffTotalS;
% ShotData=perm83(NoNavB.*DataCCP(:,TrajPartMed,CurRep,1:ShotNcc));
% ShotTraj=perm83(STraj3MMed(:,:,CurRep));
% %%
% clear Rec_CompgB0_C_RSx
% for rs=2:nRepsSets
%     CurRep=RepsSets{rs};
%     RecStr=[' -m -w 1 -S -g -d 5 ' RhoStr ' -b ' num2str(BlkSz) ' -R L:3:3:' num2str(LLR_lambda) ' '];
%     RecStr=[' -m -S -d 5 '];
%     Rec_CompgB0_C_RSx{rs}=bart(['picsS ' RecStr ScriptFN_CompgBo],...
%         Sz16CompgB0,ShotData,...
%         SensCC(:,:,:,1:ShotNcc),ShotTraj,TSBPMed,ShotTSC,...
%         ShotKerns,CompsP(1,1,1,1,1,:,:));
% %         sum(KernsPMMed(:,:,CurRep,:,:,:,:),3),CompsP(1,1,1,1,1,:,:));
% end
% Rec_CompgB0_RS_Mx=cat(4,Rec_CompgB0_C_RSx{:});
% Rec_CompgB0_RS_MXx=perm43(squeeze(sum(Rec_CompgB0_RS_Mx.*gpermute(CompsP,[4 1]),6)));
% Rec_CompgB0_RS_MXx=Rec_CompgB0_RS_MXx.*perm43(squeeze(TSCxPMedOnlyB0));
% 
% WhichTSToUse_LLR=11:90;
% 
% clear PDBase_RS_LLRx UpdatedB0Map_RS_LLRx UpdatedT2SMap_ms_RS_LLRx s_vals_RS_LLRx Fitted0_RS_LLRx PDBase0_RS_LLRx
% for rs=1
%     [PDBase_RS_LLRx(:,:,rs), UpdatedB0Map_RS_LLRx(:,:,rs), UpdatedT2SMap_ms_RS_LLRx(:,:,rs), s_vals_RS_LLRx(:,:,:,rs), Fitted0_RS_LLRx(:,:,:,rs), PDBase0_RS_LLRx(:,:,rs)]=...
%         FitToModel_MPBD1CSf(squeeze(Rec_CompgB0_RS_MXx),WhichTSToUse_LLR,dT_Med_ms,TE0_ms+FirstT_Med_ms(1));
% end
...
for rs=2:nRepsSets
    ...
    CurRepIdxs=1:3;
    RecStr=[' -m -w 1 -S -g -d 5 ' RhoStr ' -b ' num2str(BlkSz) ' -R L:3:3:' num2str(LLR_lambda) ' '];
    RecStr=[' -m -S -d 5 -R W:3:0:0.1 '];
    Rec_CompgB0_C_RS{rs}=bart(['picsS ' RecStr ScriptFN_CompgBo],...
        Sz16CompgB0,NoNavB.*DataCCP(:,TrajPartMed,CurRep(CurRepIdxs),1:nccToUse),...
        SensCC(:,:,:,1:nccToUse),STraj3MMed(:,:,CurRep(CurRepIdxs)),TSBPMed,TSCxPMedOnlyB0(:,:,1,1,1,1,:),...
        sum(KernsPMMedx(:,:, CurRepIdxs ,:,:,:,:),3),CompsP(1,1,1,1,1,:,:));
%         sum(KernsPMMed(:,:,CurRep,:,:,:,:),3),CompsP(1,1,1,1,1,:,:));
end
%%
Rec_CompgB0_RS_M=cat(4,Rec_CompgB0_C_RS{:});
Rec_CompgB0_RS_MX=perm43(squeeze(sum(Rec_CompgB0_RS_M.*gpermute(CompsP,[4 1]),6)));
Rec_CompgB0_RS_MX=Rec_CompgB0_RS_MX.*perm43(squeeze(TSCxPMedOnlyB0));

% WhichTSToUse_LLR=3:18;
WhichTSToUse_LLR=11:90;
WhichTSToUse_LLR=2:6;
WhichTSToUse_LLR=3:5;
WhichTSToUse_LLR=2:7;
...
for rs=1
    [PDBase_RS_LLR(:,:,rs), UpdatedB0Map_RS_LLR(:,:,rs), UpdatedT2SMap_ms_RS_LLR(:,:,rs), s_vals_RS_LLR(:,:,:,rs), Fitted0_RS_LLR(:,:,:,rs), PDBase0_RS_LLR(:,:,rs)]=...
        FitToModel_MPBD1CSf(squeeze(Rec_CompgB0_RS_MX),WhichTSToUse_LLR,dT_Med_ms,TE0_ms+FirstT_Med_ms(1));
end
...
SliI=SlicesToRead;
save([mainP filesep 'For_NU_MPBD3x_S' num2str(SliI) '.mat'],'CurSig','CurKernsA','CurSens','CurTraj','Fitted0_RS_LLR',...
...
for SliI=1:nSlices
    disp(SliI);
    Rec1S(SliI)=load([mainP filesep 'For_NU_MPBD3x_S' num2str(SliI) '.mat']);
end
%%
for SliI=1:nSlices
    disp(SliI);
