...
% TIdxs=[2:500:10000];
% RMsk=rand(Sz128)>0.999;
% RMsk=zeros(Sz128);
% RMsk(5:16:end,3:13:end)=1;
% [Ix1 Ix2]=find(RMsk);
% Trajm2=[Ix1.' ; Ix2.']-64;
% cCAIPIVecX=cCAIPIVecX(:,1:size(Trajm2,2));
% Trajm2=Trajm2(:,TIdxs);
% cCAIPIVecX=cCAIPIVecX(:,TIdxs);
...
nufftStruct = nufft_init(BART2Fes_NUFT_Idxs(Trajm2, Sz128),Sz128, [5 5], Sz128*2,Sz128/2); % , [0 0] st.om
...
NUFT_T=@(x,fkern) TakeTopLeftBlock(ifft2(fkern.*fft2(padarray(x,Sz128,'post'))),Sz128)*Fac;
...
%%
MM1=fftkern11.*fftkern21;
MM2=fftkern12.*fftkern22;

Mab=fftkern12.*fftkern21;
Maa=fftkern11.*fftkern22;
...
X=AHAI12;figure;subplot(1,2,1);gmontage(abs(X),[0 3000000]);subplot(1,2,2);gmontage(angle(X),[-pi pi]);title(AHAI1)
...
X=TBoth;figure;subplot(1,2,1);gmontage(abs(X),[0 3000000]);subplot(1,2,2);gmontage(angle(X),[-pi pi]);
subplot(1,2,1);title('TBoth');colorbar

grmss(AHAI12)/grmss(AHAI12-TBoth)
%%
AHAI1x1=NUFT_adj(Sig1.*conj(w1));
AHAI1x2=NUFT_adj(Sig1.*conj(w2));
AHAI2x1=NUFT_adj(Sig2.*conj(w1));
AHAI2x2=NUFT_adj(Sig2.*conj(w2));

grmss(AHAI1x1)/grmss(AHAI1x1-Tv11)
grmss(AHAI1x2)/grmss(AHAI1x2-Tv12)
grmss(AHAI2x1)/grmss(AHAI2x1-Tv21)
grmss(AHAI2x2)/grmss(AHAI2x2-Tv22)
...
% Dims: [H,W,Channels,MB,MBaux,TS,batch,whatever]
MB=1;
nTimeSegmetns=7;
nChannels=3;
nTraj=size(P,1);
...
NUFT_adjSMBTS_ITS=@(x,SensMBTSC) sum(ifft2osN(MultSpTensor(P',x,[gsize(SensMBTSC,1:2)*2,gsize(SensMBTSC,3:9)])).*conj(SN).*conj(SensMBTSC),3);
NUFT_adjSMBCTS_ITS=@(x,SensMBTSC,wb) NUFT_adjSMBTS_ITS(x.*wb,SensMBTSC);
NUFT_adj_forwSMBCTS_ITS=@(x,SensMBTSC,wa,wb) NUFT_adjSMBCTS_ITS(NUFT_forwSMBCTS(x,SensMBTSC,wa),SensMBTSC,wb);
...
NUFT_TSMBCTS_L_ITS=@(x,SensMBTSC,fkerns) sum(ifft2osN(permute54(sum(fkerns.*fft2osN(x.*SensMBTSC),4))).*conj(SensMBTSC),3);
% Verify operator normalization
ImSize=[gsize(SensMB,1:2) 1 size(SensMB,4)];
...
% imshowpair(abs(GGvSMBCTS),abs(TvSMBCTSL),'montage')

GGvSMBCTS_ITS=NUFT_adj_forwSMBCTS_ITS(IMB,SensMBTSC,wCTS,conj(wCTS));
TvSMBCTSL_ITS=NUFT_TSMBCTS_L_ITS(IMB,SensMBTSC,fftkernsTS);

GGvSMBCTS_ITSX=reshape(GGvSMBCTS_ITS,[128 128*7]);
TvSMBCTSL_ITSX=reshape(TvSMBCTSL_ITS,[128 128*7]);

imshowpair(abs(GGvSMBCTS_ITSX),abs(TvSMBCTSL_ITSX),'montage')
imshowpair(abs(GGvSMBCTS_ITSX),abs(TvSMBCTSL_ITSX))
%% Speed test
nSpeesTest=10;
nTimeSegmetns=10;
nCh=8;
