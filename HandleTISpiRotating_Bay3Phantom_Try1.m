...
OnesSens=ones(TrgSz);
%%
AcqDwellTime_us=WipMemBlock.adFree{14}/1000;

Traj=interp1(1:5001,kK(1:5001),1:(AcqDwellTime_us/GradDwellTime_us):5001-0.001);
%%
Extra=4;
Delay_us=-2.5; % positive data rightward %-6; % clockwise
kKA=[zeros(4,1); kK]; % in us
...
for i=1:numel(TrajPeaks)
    ...
    plot([TrajPeaks(i) TrajPeaks(i)],gmax(DataToPlot(TrajPeaks(i)+(-100:100),:))*[0 1],'r');
...
fgmontage(Rec1ccp)
%%
InnerShotLen=floor(nTrajToUse/4);
for i=1:4
    nccToUse=31;
    RepsToUse=1:29;
%     RepsToUse=1:10;
%     TrajPartToUse=(i-1)*10000+(1:2500);
    TrajPartToUse=(i-1)*InnerShotLen+(1:5000);
    DataCCP=permute(DataPC,[1:3 8 4:7]);
