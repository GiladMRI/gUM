...
RefPDBase0=min(RefPDBase0,grmss(RefPDBase0)*6);
RefMidEcho=RefPDBase0.*exp(-20./RefT2SM);
%%
RepSetIToShow=2;
SeveralRecs=cat(3,THLRMultiShot_RS(:,:,7,RepSetIToShow),Fitted0_RS(:,:,7,RepSetIToShow),Rec_CompgB0_RS_MX(:,:,10,RepSetIToShow),...
    Fitted0_RS_LLR(:,:,10,RepSetIToShow),CurEst0bXs(:,:,20),CurEstXs(:,:,20));
...
%%
% save('For_NU_MPBD.mat','CurSig','CurKerns','CurSens','CurTraj','CurTSB','THLRMultiShot');
% load('For_NU_MPBD.mat');
% load('For_NU_MPBD2.mat');
% % save('For_NU_MPBD2.mat','THLRMultiShot_RS','PDBase_RS','UpdatedB0Map_RS','UpdatedT2SMap_ms_RS','Fitted0_RS');
% % AcqDwellTime_us=1.1;
% THLRMultiShot=THLRMultiShot_RS(:,:,:,2);

% load('/autofs/cluster/kawin/Gilad/EPTI_and_spi68ms/meas_MID03499_FID20468_gSpi2d/For_NU_MPBD3_S10_.mat');
% load('/autofs/cluster/kawin/Gilad/EPTI_and_spi68ms/meas_MID03499_FID20468_gSpi2d/For_NU_MPBD3_S10.mat');
...
%%
% nEchosIn=size(squeeze(BaseEstX),3);
% 
% WhichTSToUs=2:nEchosIn-1;
% nWhichTSToUs=numel(WhichTSToUs);
% 
% HankelTemporalLen=2;
% [~, ~, ~,H]=ghankel(nWhichTSToUs,HankelTemporalLen,gsize(BaseEstX,1:2));
% 
% 
% In=BaseEstX(:,:,WhichTSToUs);
% nCurEchos=size(In,3);
% [ ~, s_vals, V_tmp] = batch_svd(H*In);
...
% MFac=min(abs(MFac),Medval*16).*exp(1i*angle(MFac));
% 
% R1TSCx=R1_tmp.^(perm32( (0:(nEchosIn-1))-WhichTSToUs(1)+1 ));
% Out=R1TSCx.*MFac;
% Out(isnan(Out))=0;
% PDBase=MFac.*(R1_tmp.^(-WhichTSToUs(1)+1));
% % ShowAbsAngle(Out,1,[0 Mx])
% Fitted0=R1TSCx.*MFac.*DMsk;
% ShowAbsAngle(Out.*DMsk,1,[0 Mx])
% 
% InnerTSDiff_ms=AcqDwellTime_us*nTraj/1000/(nEchosIn-1);
% UpdatedT2SMap_ms1=-InnerTSDiff_ms./log(abs(R1_tmp));
% UpdatedB0Map1=-(angle(R1_tmp)/(2*pi))/(InnerTSDiff_ms/1e3); % in Hz
%%
nEchos=80;
%%
...
subplot(2,2,4);gmontage(1./MPBD.d,[0 100]);
