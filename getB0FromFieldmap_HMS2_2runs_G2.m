...
for SliI=1:nSlices
    ...
    if(grmss(FirstEcho(:,:,:,SliI),1:3)<eps)
        SensBMM{SliI}=zeros([SnsSzB size(FirstEcho,3) 2]);
        continue;
    end
    try
        SensBMM{SliI}=RunESPIRiTForSensMapsMultiMap(FirstEcho(:,:,:,SliI),0,SnsSzB);
    catch
        SensBMM{SliI}=RunESPIRiTForSensMapsMultiMap(FirstEcho(:,:,:,SliI),24,SnsSzB);
    end
...
save([BaseP FN filesep 'Combined.mat'],'Combined');
%%
WhichEchosToUse=1:nEchos;
for i=1:nSlices
    disp(i);
    [PDBase(:,:,i), UpdatedB0Map_Hz(:,:,i), UpdatedT2SMap_ms(:,:,i), s_vals(:,:,:,i), Fitted0(:,:,:,i), PDBase0(:,:,i)]=...
        FitToModel_MPBD1CSf(Combined(:,:,i,:),WhichEchosToUse,dTEs_ms(1),TEs_ms(1));
end
%%
...
% Ctmp=grmss(Combined,4).*exp(1i*dAngle);
% CtmpS=SmoothByW(Ctmp,abs(Ctmp));
% dAngle=angle(CtmpS);
...
% B0M_HzS=SmoothByW(B0M_Hz,grmss(Combined,4));
...
save([BaseP FN filesep 'B0T2S.mat'],'SensMsk','B1','BN1','B2','B2D','B3','B4',...
    'B0M_Hz','UpdatedB0Map_Hz','UpdatedT2SMap_ms','s_vals','PDBase0','TEs_ms','dTEs_ms');
disp('Saved');