...
FN='meas_MID867_gBP_ASL_SMS_Spi_TI_VD1_ST15_FID51481';
FN='meas_MID869_gBP_ASL_SMS_Spi_TI_VD1_ST16_FID51483';
FN='meas_MID871_gBP_ASL_SMS_Spi_TI_VD1_ST17_FID51485';
FN='meas_MID873_gBP_ASL_SMS_Spi_TI_VD1_ST18_FID51487';
FN='meas_MID875_gBP_ASL_SMS_Spi_TI_VD1_ST19_FID51489';
...
FN='meas_MID883_gBP_ASL_SMS_Spi_TI_VD1_ST10_low_FID51497';
FN='meas_MID885_gBP_ASL_SMS_Spi_TI_VD1_ST13_low_FID51499';
FN='meas_MID887_gBP_ASL_SMS_Spi_TI_VD1_ST15_FID51501';
RefFldMapP='/autofs/cluster/kawin/Gilad/Skope_7May19/CRAZY_TRAJECTORIES_TWIX/meas_MID881_BP_fieldmap_9echos_2mm_Full_low_FID51495/';
...
Nres=96;
if(sTwix.hdr.Phoenix.sWiPMemBlock.adFree{13}>14)
    Nres=128;
end
Sz=[Nres Nres];
%%
...
%%
Grads=load('GAll.mat');
CurGrad=Grads.GAll(:,mod(sTwix.hdr.Phoenix.sWiPMemBlock.adFree{13}-10,5)+1,1+(Nres>96));
%%
g=CurGrad;
FOV_mm=200;
...
SensB=SensB.SensB;
SensB=imresizeBySlices(SensB,Sz);
...
FirstEcho=imresizeBySlices(FirstEcho,SnsSzB);
...
B0Q2=gflip(B0Q2,1:2);
B0Q2=imresizeBySlices(B0Q2,SnsSzB);
...
Rec1c=bart('pics -t ',BARTTrajB,DataCCP(1,:,1,1:nccToUse,1,5),SensCCP(:,:,:,1:nccToUse));
...
RecTS=bart(['picsS ' ScriptFN_TS],Sz16,DataCCP(:,:,:,1:nccToUse,:,5),SensCCP(:,:,:,1:nccToUse),STraj3,TSBP,TSCP,KernsP);
fgmontage(RecTS);MaximizeFig;
%%
RecTS_W=bart(['picsS -m -R W:3:0:1.01 ' ScriptFN_TS],Sz16,DataCCP(:,:,:,1:nccToUse,:,5),SensCCP(:,:,:,1:nccToUse),STraj3,TSBP,TSCP,KernsP);
...
RecTS_Wp=bart(['picsS -m -R W:3:0:10.01 ' ScriptFN_TS],Sz16,DataCCP(:,CurIdxs,:,1:nccToUse,:,5),SensCCP(:,:,:,1:nccToUse),STraj3p,TSBPp,TSCPp,KernsPp);
...
RecTS_2Lb=bart(['picsS -m ' RegCmd ' ' ScriptFN_TS_2L],Sz16_2L,DataCCP(:,:,:,1:nccToUse,:,5),SensCCP(:,:,:,1:nccToUse),STraj3,TSBP_L2,TSCP_L2,KernsP_L2,TSBP_L1);
...
%%
R1sx=cat(3,R1s{1:10:51});
UpdatedB0Maps=-angle(R1sx)*1e3/2/pi;
fgmontage(UpdatedB0Maps,[-300 500]);colorbar;MaximizeFig;

%%
A=load('LastRes_Low_15mm.mat');
R1s=cat(3,A.R1s{1:10:51});
UpdatedB0Maps=-angle(R1s)*1e3/2/pi;
fgmontage(UpdatedB0Maps,[-300 500]);colorbar;MaximizeFig;
gprint(0,'UpdatedB0maps.png');
...
RegCmd='-R K:3:3:1:2:1:0:5';
RecTS_2Lb_w1=bart(['picsS ' ' -W ' WarmStartFN ' -m ' RegCmd ' ' ScriptFN_TS_2L ],Sz16_2L,DataCCP(:,:,:,1:nccToUse,:,5),SensCCP(:,:,:,1:nccToUse),STraj3,TSBP_L2,TSCP_L2,KernsP_L2,TSBP_L1);
...
TimePoints_ms_L2=linspace(0,nAcqPoints*1e3/SpBW,nTS_L2);
TimePoints_ms3_L2=permute(TimePoints_ms_L2,[1 3 2]);
TSC_L2=exp(1i.*2*pi*UpdatedB0Map*(1e-3).*TimePoints_ms3_L2);
% [x y z Channels 1 TS]
...
RecTS_2LCb_LLR=bart(['picsS -m ' RegCmdLLR ' ' ScriptFN_TS_2L],Sz16_2L,DataCCP(:,:,:,1:nccToUse,:,5),SensCCP(:,:,:,1:nccToUse),STraj3,TSBP_L2,TSCP_L2,KernsP_L2,TSBP_L1);
...
Updated_TSC_L2=exp(-1i*angle(R1).*permute(0:49,[1 3 2]));
%% Compare to estimated B0
[ U_LLRa, s_LLRa, V_LLRa ] = batch_svd(H_for(TSC_L2(:,:,WhichIdxs)));
