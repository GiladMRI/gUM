...
BaseP='/autofs/cluster/kawin/Gilad/gep_Phantom/';
FN='meas_MID00892_FID42832_gre_4echo_24_26_G2';
...
BaseP='/autofs/space/daisy_001/users/data/Gilad/SCEPTI_AM_14Nov19/';
FN='meas_MID00699_FID49252_gre_4echo_314_202_G2';
FN2='meas_MID00700_FID49253_gre_4echo_415_202_G2';
...
D=gpermute(Data,[1 3 4 2 8 5 6 7]); % RO PE 1 Ch Echos Slices
nCh=size(RefDataP,3);
nEchos=size(D,5);
%%
kCalibSize=[20 20];
...
lambda=0.01;
SliI=7;
resAll=zeros([gsize(D,1:2) nCh nSlices nEchos]);
for SliI=5:nSlices
    kCalib = crop(squeeze(RefDataP(:,:,:,SliI)),[kCalibSize,nCh]);
    for EchoI=1:nEchos
        disp([SliI EchoI]);
        disp(datestr(now));
        resAll(:,:,:,SliI,EchoI) = GRAPPA(squeeze(D(:,:,1,:,EchoI,SliI)),kCalib,kSize,lambda);
    end
end
resAll=single(resAll);
save([BaseP FN filesep 'resAll.mat'],'resAll');
%%



%%
sTwix = mapVBVD([BaseP FN2 '.dat'],'removeOS','ignoreSeg','doAverage','rampSampRegrid');
sTwixX=sTwix{end};
Data=sTwixX.image();
...
mkdir([BaseP FN2]);
%%
save([BaseP FN2 filesep 'sTwixX.mat'],'sTwixX');
%%
nSlices=numel(sTwixX.hdr.Phoenix.sSliceArray.asSlice);
...
disp('ok');
%%
save([BaseP FN2 filesep 'Locs.mat'],'RotatedLocs');
disp(['Saved ' BaseP FN2 filesep 'Locs.mat']);
%%
RefDataP=permute(RefData(:,:,:,:,:,:,:,1),[1 3 2 5 4]);
% iRef=ifft2cg(RefDataP); % [RO PE Ch Sli]
D=gpermute(Data,[1 3 4 2 8 5 6 7]); % RO PE 1 Ch Echos Slices
...
resAll=single(resAll);
save([BaseP FN2 filesep 'resAll.mat'],'resAll');
