...
prismaOld=safe_params_prisma_bad();
...
[G6ms{1}, S6ms{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,3.02,true,InnerShotTime_ms,28,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle); 
...
% gPlotTraj_radm(tmp,FOV,prismaOld);MaximizeFig;
% subplot(2,3,2);cla;QuickAnalyzeTrajJumpsf(tmp.',GradDwellTime_ms,TwoPiGammaMHz,FOV_mm);setYaxis([0 3.5]);
%%
...
% [G8ms{1}, S8ms{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,4,true,InnerShotTime_ms,Gmax_mTm,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle);
...
% [G6ms{1}, S6ms{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,3.02,true,InnerShotTime_ms,28,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle); 
% [G8ms{2}, S8ms{2}]=OutInSingleInnerShotf(FOV,ResV(2),VD,2.98,InnerShotTime_ms,Gmax_mTm,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle);
...
% [G6ms{1}, S6ms{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,3.02,false,InnerShotTime_ms,28,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle); 
...
% [G8msc{1}, S8msc{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,4.2,true,InnerShotTime_ms,28.5,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle);
...
% [G12msc{1}, S12msc{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,6.7,true,InnerShotTime_ms,28.5,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle);
...
% [G16msc{1}, S16msc{1}]=OutInSingleInnerShotf(FOV,ResV(1),VD,9.0,true,InnerShotTime_ms,28.5,Smax_Tms,GradDwellTime_ms,prisma,PNSThresh,GoldenAngle);
...
%%
save('Grads2mm.mat','Grads2mm','Grads');
%% That's it
