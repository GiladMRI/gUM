...
% SB0=SM0B0./SM0;
for i=1:numel(SSigs)
    SM0B0(:,:,i)=SmoothBySlices(M0B0,SRange,SSigs(i));w
...
if(UseOS)
    ...
    QQ=bart(['splitProx -i 10000 -s 200 -d 2 -g -G 1 -f -F ' ToBARTP ' ' LS_ScriptFN],BARTS_Aop.ImSz16,1,BARTS_Aop.Others{:});
end
...
%%
fgmontage(Maps{1},[0 3])

fgmontage(Maps{4},[0 200])
fgmontage(MapsT{4},[0 200])
fgmontage(MapsTOSe{4},[0 200])