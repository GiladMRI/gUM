...
BaseFN='meas_MID00886_FID32121_gSpi2d_T13_Dw11_d110_VD1_Cor';
% BaseFN='meas_MID00918_FID32147_gSpi2d_T10_Dw11_d110_VD1_Sag';
% BaseFN='meas_MID00914_FID32145_gSpi2d_T10_Dw11_d110_VD1_32sli';
...
%     DataC=perm43(sum(perm32(ADataIsL).*permute(sccmtx(:,1:ncc),[3 4 1 2]),3));
    ...
    disp('ok SelfSens');
    save(CurSensFN,'SelfSens1');
end
%%
STraj3MFN=[mainP 'STraj3M.mat'];
if(exist(STraj3MFN,'file'))
    disp('Loading STraj3M');
    load(STraj3MFN);
else
    clear STraj3M
    for CurRep=1:(nRepsHdr-1)
        STraj=TrajM(CurRep,:);
        STraj3M(:,:,CurRep)=CTo3Rows(STraj);
    end
    save(STraj3MFN,'STraj3M');
end
%% Prepare for THLR
CurReps=1:39;
% CurReps=1:3;
RepsStr=strrep(strrep(GroupToStr( strrep(gmat2cell(num2str((CurReps).'),1),' ','')),'[','A'),']','A');

nTS_THLR=15;

TotalAcqTime_ms=AcqDwellTime_us*nTrajToUse/1000;
...
for SlicesToRead=1:nSlices
    ...
    if(exist(THLR_FN,'file')), continue; end
    ...
    DataCCP=ADatax.image(:,:,:,:,SliI,3,:,:,CurReps,:,:,:,:,:,:,:,:);
    ...
    DataCCP=permute(DataCCP(1:nTrajToUse,:,:,:,:,:,:,:,:),[4 1 2 5 6 7 8 3]).*modx(:,:,CurReps);
    DataCCP=permute(DataCCP,[1:3 8 4:7]);
    disp('read and CC sig');

    disp(['Running THLR ' datestr(now)]);
    clear THLRMultiShot;
    THLRMultiShot=bart(['picsS -w 1 -m ' RhoStr ' -R K:64:3:' num2str(THLR_lambda) ':2:1:0:6 ' ScriptFN_AllTS],Sz16_THLR,NoNavB.*DataCCP(:,:,CurReps,1:nccForTHLR),...
        SensCC(:,:,:,1:nccForTHLR),STraj3M(:,:,CurReps),NoNavB.*TSB_THLRP,1,...
        sum(KernsP_TSTHLR(:,:,CurReps,:,:,:,:),3));
    
    WhichTSToUs=2:12;
    clear PDBaseTHLR UpdatedB0MapTHLR UpdatedT2SMap_msTHLR s_valsTHLR Fitted0THLR PDBase0THLR
    [PDBaseTHLR, UpdatedB0MapTHLR, UpdatedT2SMap_msTHLR, s_valsTHLR, ~, PDBase0THLR]=...
        FitToModel_MPBD1CSf(THLRMultiShot,WhichTSToUs,dT_THLR_ms,TE0_ms+FirstT_THLR_ms);
...
%     SPDBase0THLRX=SmoothBySlices(PDBase0THLRX,[20 20],5)./SmoothBySlices(abs(PDBase0THLRX),[20 20],5);
    ...
    save(THLR_FN,'THLRMultiShot','UpdatedB0MapTHLR','UpdatedT2SMap_msTHLR','s_valsTHLR','PDBase0THLR');
    disp(['saved ' CurSRPrefix 'THLRres.mat']);
end
%%
clear RefSens DataC DataPC BARTTraj BARTTrajP ADataIsL RefMaps
clear PDBaseTHLR UpdatedB0MapTHLR UpdatedT2SMap_msTHLR s_valsTHLR Fitted0THLR PDBase0THLR
SmallVarT=50e6;
getSmallVars;
sum(SmallVarsBytes)/1e6
...
for SlicesToRead=1:nSlices
    ...
    Rec1S(SliI)=load(THLR_FN);
    THLRMultiShotS(:,:,:,SliI)=Rec1S(SliI).THLRMultiShot;
    UpdatedT2SMap_ms_THLRS(:,:,SliI)=Rec1S(SliI).UpdatedT2SMap_msTHLR;
    UpdatedB0MapTHLRS(:,:,SliI)=Rec1S(SliI).UpdatedB0MapTHLR;
    s_valsTHLRS(:,:,:,SliI)=Rec1S(SliI).s_valsTHLR;
end
% fgmontagex(gflip(THLRMultiShotS(:,:,7,ROrd),1))
%%
for SliI=1:nSlices % :nRepsSets
    disp(SliI);
    UpdatedB0Map_RSxS(:,:,SliI)=SmoothByW(UpdatedB0MapTHLRS(:,:,SliI),grmss(s_valsTHLRS(:,:,:,SliI),3));
end
fgmontagex(gflip(UpdatedB0Map_RSxS(:,:,ROrd),1),[-200 200])
%%
clear SensCCS
for SliI=1:nSlices
    SliPrefix=[mainP filesep 'Sli' num2str(SliI) '_'];
...
for SliI=1:nSlices % :nRepsSets
    ...
    [PDBase_RSS(:,:,SliI), UpdatedB0Map_RSS(:,:,SliI), UpdatedT2SMap_ms_RSS(:,:,SliI), s_vals_RSS(:,:,:,SliI), Fitted0_RSS(:,:,:,SliI), PDBase0_RSS(:,:,SliI)]=...
        FitToModel_MPBD1CSf(THLRMultiShotS(:,:,:,SliI),WhichTSToUs,dT_THLR_ms,TE0_ms+FirstT_THLR_ms);
end
...
%%
Msk=grmss(s_vals_RSS,3)>0.0001;
%%
nccToUse=15;
RepSetX=[1 13 25; 6 18 30; 4 16 28; 9 21 33];
RepSetX=repmat(RepSetX,[4 1]);
CurSens=perm95(SensCCS(:,:,:,1:nccToUse,SliIs));
% CurSens=CurSens.*perm93(Msk(:,:,SliIs));
...
%%
WhichTSToUse_Sub3D=3:7;
for SliI=1:nSlices % :nRepsSets
    disp(SliI);
    [PDBase_Sub3D(:,:,SliI), UpdatedB0Map_Sub3D(:,:,SliI), UpdatedT2SMap_ms_Sub3D(:,:,SliI), s_vals_Sub3D(:,:,:,SliI), Fitted0_Sub3D(:,:,:,SliI), PDBase0_Sub3D(:,:,SliI)]=...
        FitToModel_MPBD1CSf(Rec_CompgB0_RSS_MX(:,:,SliI,:),WhichTSToUse_Sub3D,dT_Med_ms,TE0_ms+FirstT_Med_ms);
end
%%
save([mainP filesep 'Rec_CompgB0_RSS_MX.mat'],'Rec_CompgB0_RSS_MX');
%%
SmallVarT=50e6;
getSmallVars;
sum(SmallVarsBytes)/1e6
...
save([mainP filesep 'For_NU_MPBD3_S' num2str(SliI) '.mat'],'CurSig','CurKernsA','CurSens','CurTraj','Fitted0_RS_LLR',...
    'UpdatedB0Map_RS_LLR','UpdatedT2SMap_ms_RS_LLR','TotalAcqTime_ms','TE0_ms','THLRMultiShot_RS','Fitted0_RS',...
    'Rec_CompgB0_RS_MX','sccmtx');
%%
...
fgmontagex(UpdatedB0Map_RS_LLRS(:,:,2,ROrd),[-200 200]);title('From LLR 3-multishot')
...
CurB0M=UpdatedB0Map_RS_LLRS(:,:,2,SliI);
...
St.SessionNameBase=[mainP filesep MIDStr '_ME_S' num2str(SliI)];
...
disp('Prepared script for MLN 1-shot runs');
%%
...
St.nToLoad=10000;
St.TimePointsForRec_ms=TS_TimePointsForRec_Str;
St.SessionNameBase=[MIDStr '_ME_3S' num2str(SliI)];
St.RealDataFN=[BaseOutDir 'RealDataForNN.mat'];
St.BaseTSDataP=BaseOutDir;
St.BaseNUFTDataP=BaseOutDir;
% if(strcmp(HostName,'tiger'))
...
disp('Prepared for MLN 3-shot');
%%
MSFN=[BaseBaseOutP 'MLNRun3'];
fid=fopen(MSFN,'w+');
% fprintf(fid,'#!/bin/bash\r\n');
fprintf(fid,'# MLN run\r\n');
fprintf(fid,'echo "Starting!"\r\n');
for i=1:numel(MSLines3)
    fprintf(fid,'%s\n',MSLines3{i});
end
fclose(fid);
[status,msg,msgID] = fileattrib(MSFN,'+x');
disp('Prepared script for MLN 3-shot runs');
%%
