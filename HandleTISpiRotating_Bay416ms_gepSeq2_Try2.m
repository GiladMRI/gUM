...
%%
CurData=zeros([1,nTrajToUse,size(RepSetX,2),nccToUse,1,1,1,1,nSliIs]);
for i=1:nSliIs
...
Rec_CompgB0_C_RSS=bart(['picsS ' RecStr ScriptFN_CompgBo],...
    ...
    CurKerns,CompsP);
%%
% Sz16CompgB0SX=Sz16CompgB0S;
% SubSliIs=50:55;
% Sz16CompgB0SX(9)=numel(SubSliIs);
% Rec_CompgB0_C_RSSA=bart(['picsS ' RecStr ScriptFN_CompgBo],...
%     Sz16CompgB0SX,CurData(:,:,:,:,:,:,:,:,SubSliIs),...
%     CurSens(:,:,:,:,:,:,:,:,SubSliIs),CurSTraj(:,:,:,:,:,:,:,:,SubSliIs),TSBPMed,TSCxPMedOnlyB0S(:,:,:,:,:,:,:,:,SubSliIs),...
%     CurKerns(:,:,:,:,:,:,:,:,SubSliIs),CompsP);
% 
% Rec_CompgB0_RSS_M=cat(4,Rec_CompgB0_C_RSSA);
% Rec_CompgB0_RSS_MX=perm43(squeeze(sum(Rec_CompgB0_RSS_M.*gpermute(CompsP,[4 1]),6)));
% Rec_CompgB0_RSS_MX=Rec_CompgB0_RSS_MX.*perm43(squeeze(TSCxPMedOnlyB0S(:,:,:,:,:,:,:,:,SubSliIs)));
% 
% WhichTSToUs=2:(nTS_THLR-1);
% [PDBase_X, UpdatedB0Map_X, UpdatedT2SMap_ms_X, s_vals_X, Fitted0_X, PDBase0_X]=...
%         FitToModel_MPBD1CSf(perm43(Rec_CompgB0_RSS_MX),WhichTSToUs,dT_THLR_ms,TE0_ms+FirstT_THLR_ms);
...
%%
save([mainP filesep 'TVRec7_4comp.mat'],'UpdatedT2SMap_ms_TV7','Fitted0_TV7');
%%
