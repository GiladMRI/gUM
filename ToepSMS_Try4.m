...
SensMB=double(permute(SensCC(:,:,:,[10 20]),[2 3 1 4]));
...
% MultSpTensor=@(Sp,x,Sz) reshape(Sp*reshape(x,size(x,1),prod(gsize(x,2:10))),Sz);
MultSpTensor=@(Sp,x,Sz) reshape(Sp*reshape(x,size(Sp,2),[]),Sz);

% NUFT_forwSMB=@(x,SensMB) reshape(P*reshape(fft2(padarray((x.*SN).*SensMB,Sz128,'post')),prod(Sz128)*4,[])*Fac,[],size(SensMB,3),size(SensMB,4));
% NUFT_forwSMB=@(x,SensMB) reshape(P*reshape(fft2((x.*SN).*SensMB,Sz128(1)*2,Sz128(2)*2),prod(Sz128)*4,[])*Fac,[],size(SensMB,3),size(SensMB,4));
NUFT_forwSMB=@(x,SensMB) MultSpTensor(P,fft2((x.*SN).*SensMB,Sz128(1)*2,Sz128(2)*2)*Fac,[nTraj,gsize(SensMB,3:4)]);

% NUFT_adjSMB=@(x,SensMB) sum((TakeTopLeftBlock(ifft2(reshape(P'*x,Sz128(1)*2,Sz128(2)*2,size(SensMB,3),size(SensMB,4))),Sz128).*conj(SN)).*conj(SensMB),3);
NUFT_adjSMB=@(x,SensMB) sum((TakeTopLeftBlock(ifft2(MultSpTensor(P',x,[Sz128*2,gsize(SensMB,3:4)])),Sz128).*conj(SN)).*conj(SensMB),3);
...
NUFT_TSMBC=@(x,SensMB,fkerns) permute54(sum(sum(TakeTopLeftBlock(ifft2(fkerns.*fft2(x.*SensMB,Sz128(1)*2,Sz128(2)*2)),Sz128).*conj(SensMB),3),4));
