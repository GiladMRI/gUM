function [x,flag,relres,iter,resvec,resveccg] = symmlqg(A,b,tol,maxit,M1,M2,x0,varargin)
...
[atype,afun,afcnstr] = iterchkg(A); % ggg
...
if ((nargin >= 5) && ~isempty(M1))
    existM1 = true;
    [m1type,m1fun,m1fcnstr] = iterchkg(M1); % ggg
...
if ((nargin >= 6) && ~isempty(M2))
    existM2 = true;
    [m2type,m2fun,m2fcnstr] = iterchkg(M2); % ggg
...
r = b - iterappg('mtimes',afun,atype,afcnstr,x,varargin{:});
...
vold = r;
if existM1
    u = iterappg('mldivide',m1fun,m1type,m1fcnstr,vold,varargin{:}); % ggg
    if ~all(isfinite(u))
        flag = 2;
        relres = normr / n2b;
        iter = 0;
...
if existM2
    v = iterappg('mldivide',m2fun,m2type,m2fcnstr,u,varargin{:}); % ggg
    if ~all(isfinite(v))
        flag = 2;
        relres = normr / n2b;
        iter = 0;
...
vv = v / beta1;
wbar = vv;
v = iterappg('mtimes',afun,atype,afcnstr,vv,varargin{:}); % ggg
alpha = real(vv' * v); % ggg added real
...
%     alpha = vv' * v;
    ...
    if (normr <= tolb || stag >= maxstagsteps || moresteps)
        ...
        r = b - iterappg('mtimes',afun,atype,afcnstr,x,varargin{:}); % ggg
        normr_act = norm(r);
    ...
    if (normrcg <= tolb || stag >= maxstagsteps || moresteps)
        ...
        r = b - iterappg('mtimes',afun,atype,afcnstr,xcg,varargin{:}); % ggg
        normrcg_act = norm(r);
        if (normrcg_act <= tolb)
            x = xcg;
            flag = 0;
...
if (flag == 0)
    ...
    r_comp = b - iterappg('mtimes',afun,atype,afcnstr,xmin,varargin{:}); % ggg
