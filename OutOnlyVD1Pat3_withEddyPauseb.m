...
% GmsA=OutSingleInnerShotPNSf(FOV,ResV(1),VD,17.7,true,Gmax_mTm,135,GradDwellTime_ms);
% GmsA=OutSingleInnerShotPNSf(FOV,ResV(1),VD,17.2,true,Gmax_mTm,135,GradDwellTime_ms);
GmsA=OutSingleInnerShotPNSf(FOV,ResV(1),VD,17.1,true,Gmax_mTm,135,GradDwellTime_ms);


k=cumsum([0; GmsA]); % mT/m*ms * 2*pi*MHz/T = rad/m
% kx=interp1(1:numel(k),k,linspace(1,numel(k),1601));
kx=interp1(1:numel(k),k,linspace(1,numel(k),1551));
Gms=diff(kx).';
Gms=[Gms; zeros(50,1)];
...
GAll=[GAll GPAT3];
save('GAll1p9mmVD1PAT3Pauseb.mat','GAll');
%%
WriteGradFile([BasePG 'Traj11'],G1,'PAT3VD1 only out 16ms no rotation');
WriteGradFile([BasePG 'Traj12'],G2,'PAT3VD1 only out 16ms with rotation');