...
%%
nccToUse=15;
RecTS_W=bart(['picsS -m -R W:3:0:0.1 ' ScriptFN_TS],Sz16,DataCCP(:,:,:,1:nccToUse,:,5),SensCCP(:,:,:,1:nccToUse),STraj3,TSBP,TSCP,KernsP);
if(ShowFigs)
    fgmontage(RecTS_W);MaximizeFig;ylabel(nccToUse);
end
save([TrgP 'RecTS_W.mat'],'RecTS_W');
%% Work on part of the data
nccToUse=15;

StartPoints=[0 25000];
clear RecTS_Wp
for si=1:numel(StartPoints)
    % CurIdxs=0+(1:25000);
    CurIdxs=StartPoints(si)+(1:25000);
    ...
    RecTS_Wp(:,:,si)=bart(['picsS -m -R W:3:0:10.01 ' ScriptFN_TS],Sz16,DataCCP(:,CurIdxs,:,1:nccToUse,:,5),SensCCP(:,:,:,1:nccToUse),STraj3p,TSBPp,TSCPp,KernsPp);
end
if(ShowFigs)
    fgmontage(RecTS_Wp);MaximizeFig;ylabel(nccToUse);
end
save([TrgP 'RecTS_Wp.mat'],'RecTS_Wp');
%%
% RecTS_Wp1=RecTS_Wp;
% RecTS_Wp2=RecTS_Wp;
% %%
...
if(ShowFigs)
    fgmontage(RecTS_2LbY(:,:,1:9:end));MaximizeFig;title(RegCmd);ylabel(nccToUse);
end
save([TrgP 'RecTS_2LbY.mat'],'RecTS_2LbY');
...
if(ShowFigs)
    fgmontage(RecTS_2LCbY(:,:,1:9:end));MaximizeFig;title(RegCmd);ylabel(nccToUse);xlabel([num2str(nComponents_L1) ' Components']);
end
save([TrgP 'RecTS_2LCbY.mat'],'RecTS_2LCbY');
%%
end % Files
