...
BaseFN='meas_MID75_gBP_ep2d_MB_U24_VD11_C100_OutInOut_FID27278';
...
ScanP='/media/deni/c78a9273-3214-4387-9f72-4cdc3adef255/OnSK_17Aug18/';
RefFldMapP='/media/deni/c78a9273-3214-4387-9f72-4cdc3adef255/OnSK_17Aug18/meas_MID35_BP_fieldmap_5echosX_FID27238/';
BaseFN='meas_MID68_gBP_ep2d_MB_U24_VD11_C100_OutInOut_FID27271';
...
%%
WhichE=1;
Echo2P=[ScanP BaseFN '_E' num2str(WhichE)];
mkdir([Echo2P]);

system(['sudo chmod +777 -R ' Echo2P]);
%% Read raw
AData = mapVBVD(FN);
% ADataI=AData.image();
...
%%
save([Echo2P filesep 'Data.mat'],'ADataIsL','AData');
disp(['Saved ' [Echo2P filesep 'Data.mat']]);
%%
...
%%
gprint(get(gcf,'Number'),[Echo2P filesep 'Traj'],[]) 
close(gcf);
save([Echo2P filesep 'Traj.mat'],'BARTTrajMS');
disp('Saved traj fig');
%% Summary P2
...
gStruct2txt(SummaryStr,[Echo2P filesep fileName]);
...
save([Echo2P filesep 'TrajForNUFT.mat'],'Trajm2','SN','Kd','P');
...
ylabel(YLbl);
%%
gprint(get(gcf,'Number'),[Echo2P filesep YLbl '_BARTRecon_NoB0_W' num2str(Lambda) T2SCompStr],[]) 
close(gcf);
save([Echo2P filesep YLbl '_BARTRecon_NoB0_W' num2str(Lambda) T2SCompStr '.mat'],'Rec');
disp(['Saved ' [Echo2P filesep YLbl '_BARTRecon_NoB0_W' num2str(Lambda) T2SCompStr '.mat']]);
%% All timepoint
if(nReps>20)
    ...
    save([Echo2P filesep 'BARTRecon_NoB0_W' num2str(Lambda) T2SCompStr '.mat'],'RecBART');
end
...
%%
gprint(get(gcf,'Number'),[Echo2P filesep YLbl '_Params'],[]) 
close(gcf);
disp(['printed fig to file' [Echo2P filesep YLbl '_Params']]);
...
%%
fgmontage(RecS)
gprint(get(gcf,'Number'),[Echo2P filesep 'BARTRecon_AllS_NoB0_W' num2str(Lambda) T2SCompStr],[]) 
close(gcf);
...
close(gcf);
save([Echo2P filesep 'BARTRecon2Maps_AllS_NoB0_W' num2str(Lambda) T2SCompStr '.mat'],'RecSMM');
%%
SliI=7;
SliIs=[SliI SliI+nSlicesNoMB];
...
gprint(get(gcf,'Number'),[Echo2P filesep YLbl 'TS'],[]) 
close(gcf);
%% GPU TS
gamma=42.5774806;
...
disp('ok');
%%
SliIP=[Echo2P filesep YLbl filesep];
mkdir(SliIP);
...
ylabel(YLbl);
%%
gprint(get(gcf,'Number'),[Echo2P filesep YLbl '_' XLbl T2SCompStr],[]) 
close(gcf);
save([Echo2P filesep YLbl '_' XLbl T2SCompStr '.mat'],'im_res');
disp(['Saved' Echo2P filesep YLbl '_' XLbl T2SCompStr '.mat']);
%%
...
CurOurP=[Echo2P filesep];
mkdir(CurOurP);
CurOurPSli=[Echo2P filesep 'Sli' num2str(SliI,'%02d') filesep];
mkdir(CurOurPSli);
...
ParamsS.BaseNUFTDataP=[Echo2P filesep];
...
St.LoadAndRunOnData_Prefix=[Echo2P filesep 'RealData' filesep 'Sli' num2str(SliI) '_r'];
St.LoadAndRunOnData_OutP=[Echo2P filesep 'RealDataOut' filesep 'Sli' num2str(SliI,'%02d') filesep];
...
save([Echo2P filesep 'RecTSAll.mat'],'RecTSAll');
...
xlabel(['Daubechies_TI lam ' num2str(lam) ' splitWeight ' num2str(splitWeight)]);
%%
gprint(get(gcf,'Number'),[Echo2P filesep YLbl '_L1ESPIRiT_B0_lam' num2str(lam) T2SCompStr '_CC' num2str(ncc)],[]) 
close(gcf);
save([Echo2P filesep YLbl '_L1ESPIRiT_B0_lam' num2str(lam) T2SCompStr '_CC.mat'],'resL1ESPIRiTCC1');
disp(['Saved ' Echo2P filesep YLbl '_L1ESPIRiT_B0_lam' num2str(lam) T2SCompStr '_CC.mat']);
%%
...
%% All reps
CurRealDataP=[Echo2P filesep 'RealData' filesep];
mkdir(CurRealDataP);
for SliI=WhichSlices
    for r=1:nReps
        disp([SliI r]);
...
CurRealDataOutP=[Echo2P filesep 'RealDataOut' filesep];
...
save([Echo2P filesep 'TSBS_TSCS.mat'],'TSBS','TSCS','ErrTSS');
