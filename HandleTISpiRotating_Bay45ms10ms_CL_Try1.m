aScanP='/media/deni/bigdrive/SPIRAL_ASL/S03/RawData/';
...
for i=1:4
    TrajPartToUse=(i-1)*InnerShotLen+(1:2000);
    InnerShotTraj{i}=BARTTrajP(:,TrajPartToUse,RepsToUse);
    InnerShotData{i}=DataCCP(:,TrajPartToUse,RepsToUse,1:nccToUse);
%     Rec1ccpM(:,:,i)=bart('pics -m -R W:3:0:0.00010 -t ',BARTTrajP(:,TrajPartToUse,RepsToUse),DataCCP(:,TrajPartToUse,RepsToUse,1:nccToUse),SensCC(:,:,:,1:nccToUse));
end
InnerShotTraj=cat(8,InnerShotTraj{:});
InnerShotData=cat(8,InnerShotData{:});
Rec1ccpMt=bart('pics -m -R W:3:0:0.0000001 -t ',InnerShotTraj,InnerShotData,SensCC(:,:,:,1:nccToUse));
...
fgmontage(UpdatedB0Map,[-100 100]);
...
tic;RecgD_W=bart(['picsS -m ' RegStr ' ' ScriptFN_TS],Sz16,DataCCP(:,BigTrajPartToUse,CurRep,1:nccToUse),SensCC(:,:,:,1:nccToUse),STraj3(:,BigTrajPartToUse),TSBP(:,BigTrajPartToUse,:,:,:,:,:,:,:),TSCxP,KernsP);
t=toc;
fgmontage(RecgD_W);MaximizeFig;xlabel(nccToUse);ylabel(t);title(['Single shot, given B_0,T_2^* estimate, ' RegStr]);
%%
TightMask=s_LLR(:,:,1)>0.002;
% se = strel('disk', 3);
