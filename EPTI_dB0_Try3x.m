BaseFP='/autofs/cluster/kawin/Gilad/EPTI_and_spi68msx_on_CL/';

% EPTIRecP='/autofs/cluster/kawin/Gilad/EPTI_and_spi68ms_on_CL/data/Recon/Human/';
EBaseP='/autofs/cluster/kawin/Gilad/EPTI_and_spi68msx_on_CL/';
R=load([EBaseP 'meas_MID00874_FID32109_ep2d_ge_EPTI_1p9_3and5shot_calib_FirstRep_Raw.mat']);

OutP=[EBaseP 'meas_MID00876_FID32111_ep2d_ge_EPTI_1p9_3shot_4dyns/'];

% parameters=load('/autofs/cluster/kawin/Gilad/EPTI_and_spi68msx_on_CL/data/Data_acq/meas_prot_Human_SMS1_1p9_3SHOT_GE.mat');
...
GRE_Ref_FN='meas_MID00856_FID32091_gre_4echo_32_22';
...
%%
SRefB0r=squeeze(EMapsX(:,:,3,:));
SRefB0r=permute(SRefB0r,[2 1 3]);
%%
...
% SliI=16;
% [4 6 9 12]
for SliI=1:16 % [4 6 9 12]% [1:8 10:32]
...
%     RecSubspaceTDPE2ROS(:,:,:,SliI)=squeeze(RecSubspaceTDPE2ROX);
    ...
    UpdatedT2SMap_ms_SubS(:,:,SliI)=UpdatedT2SMap_ms_Sub;
    PDBase_SubS(:,:,SliI)=PDBase_Sub;
    PDBase0_SubS(:,:,SliI)=PDBase0_Sub;
...
%%
[PDBase_G, UpdatedB0Map_Hz_G, UpdatedT2SMap_ms_G, s_vals_G, Fitted0_G, PDBase0_G]=...
        FitToModel_MPBD1CSf(RecAllEchosS,WhichEchosToUse,ES_ms,FirstTE_ms);

save('EPTIx_GSS2.mat','EMapsX','UpdatedT2SMap_ms_SubS','FirstTE_ms','WhichEchosToUse','ES_ms',...
    'RecSubspaceS','Fitted0_SubS','EPTIRefS','RecAllEchosS','UpdatedT2SMap_ms_G');
%%
SliIsToShow=[4 6 9 12];
SliIsToShow=1:16;
Recs=cat(5,rot90(RecAllEchosS(:,:,:,SliIsToShow)),RecSubspaceS(:,:,:,SliIsToShow),rot90(EPTIRefS(:,:,:,SliIsToShow)));
T2S=cat(4,rot90(UpdatedT2SMap_ms_G(:,:,SliIsToShow)),UpdatedT2SMap_ms_SubS(:,:,SliIsToShow),gflip(rot90(squeeze(EMapsX(:,:,4,SliIsToShow)),3),2));
T2S=min(200,max(0,abs(T2S)));

Msk=abs(squeeze(Recs(:,:,40,:,:)))>0.3;
Msk=RemoveSmallComponentsBySlices(Msk,7);
T2Sm=Msk.*T2S;
fgmontagex(perm43(T2Sm(1:110,10:110,:,:)),[0 200]);colormap hot
fgmontagex(perm43(squeeze(Recs(1:110,10:110,44,:,:))));caxis(caxis/1.1);
%%
fgmontagex(perm43(T2Sm(1:110,10:110,2:5:end,:)),[0 200]);colormap hot
fgmontagex(perm43(squeeze(Recs(1:110,10:110,44,2:5:end,:))));caxis(caxis/1.1);
